...
trait Ledger
{
    ...
    {
        if ($dateFormat = Config::get('accountant.ledger.date_format')) {
    ...
    /**
     * Get a user prefixed attribute key.
     *
     * @param string $attribute
     *
     * @return string
     */
    protected function getUserPrefixedKey(string $attribute): string
    {
        return sprintf('%s_%s', Config::get('accountant.user.prefix', 'user'), $attribute);
    }
    ...
    public function compile(): array
    {
        ...
        $this->data = [
            ...
            $this->getUserPrefixedKey('id')   => $this->getAttributeFromArray($this->getUserPrefixedKey('id')),
            $this->getUserPrefixedKey('type') => $this->getAttributeFromArray($this->getUserPrefixedKey('type')),
        ];

        if ($this->user instanceof Identifiable) {
            foreach ($this->user->getArrayableAttributes() as $attribute => $value) {
                $this->data[$this->getUserPrefixedKey($attribute)] = $value;
            }
        }
